...
def calc_min_max_coordinate(image: np.ndarray, homography: np.ndarray) -> Tuple[int, int, int, int]:
...
def calc_image_size(images: List[np.ndarray], homographies: np.ndarray) -> Tuple[int, int, Tuple[int, int]]:
    min_x, min_y = 0, 0
    max_y, max_x, _ = images[0].shape

    for image, homography in zip(images[1:], homographies):
        min_x_tmp, min_y_tmp, max_x_tmp, max_y_tmp = calc_min_max_coordinate(image, homography)
...
def composite_homographies(homographies: np.ndarray) -> np.ndarray:
    ...
    for i, composited_homography in enumerate(homographies):
        ...
        composited_homographies = np.vstack((composited_homographies, composited_homography))
